...
class TestPropertyInspection(unittest.TestCase):
    ...
    def test_get_agent_inspection_info_from_database(self):
        agent_id = 2  # Replace with your agent_id
        self.mock_cursor.fetchall.return_value = [("123", 1, 2, 3, "2023-10-12")]
        result = self.property_inspection.get_agent_inspection_info_from_database(agent_id)
        self.assertEqual(result, [("123", 1, 2, 3, "2023-10-12")])

    def test_get_tenant_inspection_info_from_database(self):
        tenant_id = 2  # Replace with your agent_id
        self.mock_cursor.fetchall.return_value = [("123", 1, 2, 3, "2023-10-12")]
        result = self.property_inspection.get_tenant_inspection_info_from_database(tenant_id)
        self.assertEqual(result, [("123", 1, 2, 3, "2023-10-12")])
