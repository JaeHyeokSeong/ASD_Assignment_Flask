class TenantRequestForm:
    ...
    def get_request_forms_by_agent_id(self, agent_id):
        select_query = "SELECT t.* FROM tenantRequestForm AS t INNER JOIN leaseapplication AS l ON t.leaseApp_id = l.leaseApp_id INNER JOIN property AS p ON l.property_id = p.property_id WHERE p.agent_id = %s"
    ...
    def get_request_forms_by_tenant_id(self, tenant_id):
        select_query = "SELECT t.* FROM tenantRequestForm AS t INNER JOIN leaseapplication AS l ON t.leaseApp_id = l.leaseApp_id INNER JOIN property AS p ON l.property_id = p.property_id WHERE p.tenant_id = %s"
